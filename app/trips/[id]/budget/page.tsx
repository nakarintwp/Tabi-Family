...
type Expense = { id: string; amount: number; currency: string; category: string; note: string | null; paid_at: string };
...
export default async function BudgetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell"><div className="container"><AppHeader />
    <div className="planner-topbar"><Link href={`/trips/${id}`} className="back-link">‹ Dashboard</Link><span className="planner-counter">V8.2 Budget</span></div>
    <section className="planner-hero v8-budget-hero"><div><span className="eyebrow">BUDGET CONTROL</span><h1>💴 Expense & Budget</h1><p>{trip.title} · แยก JPY/THB เพื่อไม่ต้องพึ่ง API ค่าเงิน</p></div><Link className="btn btn-secondary" href={`/trips/${id}/bookings`}>Booking Center</Link></section>
