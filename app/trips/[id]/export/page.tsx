...
export default async function ExportPage({ params }: { params: Promise<{ id:string }> }) {
  ...
  const { data: trip } = await supabase.from("trips").select(`
    id,title,start_date,end_date,cities,pace,budget,currency,created_at,updated_at,cover_style,cover_emoji,cover_tagline,template_key,
    trip_members(id,name,member_type,age,walking_level,needs,interests,dietary_preferences,avoid_stairs,needs_frequent_rest,stroller,mobility_notes,notes),
    ...
    trip_wishlist(id,place_key,title,city,category,emoji,summary,maps_url,latitude,longitude,child_friendly,senior_friendly,is_outdoor,duration_minutes,notes),
    transport_segments(id,day_id,mode,operator,service_name,origin,destination,departure_time,arrival_time,reservation_required,booking_reference,seat,notes,sort_order)
  `).eq("id", id).single();
  if (!trip) notFound();
  ...
  return <main className="shell print-shell"><div className="container"><div className="no-print"><AppHeader /><Link className="back-link" href={`/trips/${id}/more`}>‹ More</Link></div>
    ...
    <section className="section print-section"><h2>Transport</h2>{transports.length ? transports.map((t:any)=><div className="print-line" key={t.id}><strong>{t.departure_time?.slice(0,5)||"—"} · {t.origin} → {t.destination}</strong><span>{t.service_name || t.operator || t.mode}{t.seat ? ` · Seat ${t.seat}` : ""}</span></div>) : <p className="muted">ยังไม่มี Transport Segment</p>}</section>
