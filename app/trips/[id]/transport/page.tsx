...
type TransportSegment = { id: string; day_id: string | null; mode: string; operator: string | null; service_name: string | null; origin: string; destination: string; departure_time: string | null; arrival_time: string | null; reservation_required: boolean; booking_reference: string | null; seat: string | null; notes: string | null; distance_km?: number | null; toll_jpy?: number | null; fuel_jpy?: number | null; parking_jpy?: number | null; rest_stop?: string | null; winter_ready?: boolean | null };
...
function SegmentFields({ segment, days, defaultMode = "train" }: { segment?: Partial<TransportSegment>; days: TransportDay[]; defaultMode?: string }) {
  return <>
    ...
    {(segment?.mode === "car" || defaultMode === "car") && <div className="driving-metric-form"><div className="grid2"><input className="input" type="number" min="0" step="0.1" name="distance_km" defaultValue={segment?.distance_km ?? ""} placeholder="ระยะทาง km"/><input className="input" type="number" min="0" step="1" name="toll_jpy" defaultValue={segment?.toll_jpy ?? ""} placeholder="ทางด่วน/ETC ¥"/></div><div className="grid2"><input className="input" type="number" min="0" step="1" name="fuel_jpy" defaultValue={segment?.fuel_jpy ?? ""} placeholder="น้ำมันประมาณ ¥"/><input className="input" type="number" min="0" step="1" name="parking_jpy" defaultValue={segment?.parking_jpy ?? ""} placeholder="ที่จอดรถ ¥"/></div><input className="input" name="rest_stop" defaultValue={segment?.rest_stop || ""} placeholder="จุดพัก / Service Area"/><label className="check-row"><input type="checkbox" name="winter_ready" defaultChecked={Boolean(segment?.winter_ready)}/><span>ตรวจ Snow tire / winter equipment แล้ว</span></label></div>}
  ...
  return <main className="shell"><div className="container"><AppHeader />
    ...
    <section className="transport-list">
      {typedSegments.length ? typedSegments.map((segment: TransportSegment) => { const meta = modeMeta[segment.mode] || modeMeta.other; const dayIndex = days.findIndex((d: TransportDay) => d.id === segment.day_id); const isCar = segment.mode === "car"; return <article className={`transport-card ${isCar ? "rental-transport-card" : ""}`} key={segment.id}>
        ...
        <div className="transport-main"><span className="activity-label">{dayIndex >= 0 ? `DAY ${dayIndex + 1} · ` : ""}{meta.label}{segment.operator ? ` · ${segment.operator}` : ""}</span><h2>{segment.origin} → {segment.destination}</h2><div className="transport-times"><strong>{segment.departure_time?.slice(0,5) || "—"}</strong><span>→</span><strong>{segment.arrival_time?.slice(0,5) || "—"}</strong></div>{segment.service_name && <p>{isCar ? `รถ/คลาส: ${segment.service_name}` : segment.service_name}</p>}<div className="tag-row">{segment.reservation_required && <span className="mini-tag">Reservation</span>}{segment.seat && <span className="mini-tag">{isCar ? segment.seat : `Seat ${segment.seat}`}</span>}{segment.booking_reference && <span className="mini-tag">Ref {segment.booking_reference}</span>}{isCar && segment.distance_km && <span className="mini-tag">{Number(segment.distance_km)} km</span>}{isCar && segment.winter_ready && <span className="mini-tag">Winter ✓</span>}</div>{isCar && (segment.toll_jpy || segment.fuel_jpy || segment.parking_jpy) && <p className="small">ETC ¥{Number(segment.toll_jpy||0).toLocaleString()} · Fuel ¥{Number(segment.fuel_jpy||0).toLocaleString()} · Parking ¥{Number(segment.parking_jpy||0).toLocaleString()}</p>}{isCar && segment.rest_stop && <p className="small">🅿 {segment.rest_stop}</p>}{segment.notes && <p className="muted small">{segment.notes}</p>}{isCar && <div className="rental-inline-note">ตรวจเอกสารขับรถ · ETC/ทางด่วน · Snow tire เมื่อมีหิมะ · ที่จอดรถ · น้ำมัน</div>}</div>
