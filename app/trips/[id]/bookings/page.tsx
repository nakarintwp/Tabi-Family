...
export default async function BookingCenterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell"><div className="container"><AppHeader />
    <div className="planner-topbar"><Link href={`/trips/${id}`} className="back-link">‹ Dashboard</Link><span className="planner-counter">V8.1 Booking Center</span></div>
    <section className="planner-hero v8-booking-hero"><div><span className="eyebrow">RESERVATION CONTROL</span><h1>🎫 Booking & Reservation</h1><p>{trip.title} · รวมเลขจอง เวลา สถานะชำระเงิน และ Confirmation</p></div><Link className="btn btn-secondary" href={`/trips/${id}/documents`}>Documents</Link></section>
