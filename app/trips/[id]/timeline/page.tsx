...
export default async function TimelinePage({ params }: { params: Promise<{ id:string }> }){
  ...
  return <main className="shell"><div className="container"><AppHeader/><div className="planner-topbar"><Link href={`/trips/${id}/master-plan`} className="back-link">‹ Plan</Link><span className="planner-counter">V9.1 Unified Timeline</span></div><section className="planner-hero timeline-hero"><div><span className="eyebrow">REAL BOOKING TIMELINE</span><h1>🕒 Trip Timeline</h1><p>{trip.title} · Activity + Transport + Booking เรียงตามวันและเวลา</p></div><Link className="btn btn-secondary" href={`/trips/${id}/master-plan`}>Master Plan</Link></section>
