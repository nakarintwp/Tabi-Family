...
export default async function ImportBookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canEdit) return <main className="shell"><div className="container"><AppHeader/><div className="planner-topbar"><Link href={`/trips/${id}/wallet`} className="back-link">‹ Wallet</Link></div><div className="error-box">Viewer สามารถดู Booking ได้ แต่ไม่สามารถนำเข้า/แก้ไขข้อมูล</div></div><BottomNav active="/wallet" tripId={id} /></main>;
  ...
  return <main className="shell"><div className="container"><AppHeader/>
    <div className="planner-topbar"><Link href={`/trips/${id}/wallet`} className="back-link">‹ Wallet</Link><span className="planner-counter">V10.8 OCR + Smart Link</span></div>
