...
export default async function ImportBookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell"><div className="container"><AppHeader/>
    <div className="planner-topbar"><Link href={`/trips/${id}/bookings`} className="back-link">‹ Booking Center</Link><span className="planner-counter">V10.0–10.1</span></div>
    <section className="planner-hero import-booking-hero"><div><span className="eyebrow">AUTO IMPORT + SMART LINK</span><h1>📥 Booking Import</h1><p>{trip.title} · วิเคราะห์ข้อมูลในเครื่องก่อน แล้วให้คุณตรวจยืนยันทุกช่องก่อนบันทึก</p></div><Link className="btn btn-secondary" href={`/trips/${id}/documents`}>Trip Documents</Link></section>
    <section className="section zero-cost-banner"><div className="zero-cost-icon">¥0</div><div><strong>ไม่ส่งเอกสารไป AI API</strong><p>ตัววิเคราะห์ใช้ rule-based local parsing สำหรับ text/PDF text layer และข้อมูลชื่อไฟล์ รูปภาพจะไม่ทำ OCR อัตโนมัติ เพื่อคุมค่าใช้จ่ายและความเป็นส่วนตัว</p></div></section>
